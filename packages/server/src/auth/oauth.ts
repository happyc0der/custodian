 ...
 * account linking requires (see docs/platform-notes.md #5):
 *
...
export function createOAuthRouter(deps: OAuthDeps): Router {
  ...
  const validResource = (r: string | undefined) =>
    !r || r.replace(/\/+$/, '') === resourceUrl(config);
  ...
  for (const p of [
    ...
    router.get(p, (_req, res) =>
      res.set('cache-control', 'public, max-age=300').json(protectedResourceMetadata(config)),
    );
  }
  router.get('/.well-known/jwks.json', (_req, res) =>
    res.set('cache-control', 'public, max-age=300').json(keys.jwks),
  );
  ...
  router.get('/authorize', async (req, res) => {
    ...
    if (!client_id || client_id !== config.oauthClientId)
      return oauthError(res, 400, 'invalid_client', 'Unknown client_id');
    if (!redirect_uri || !validRedirect(redirect_uri))
      return oauthError(
        res,
        400,
        'invalid_request',
    ...
    if (!code_challenge || (code_challenge_method ?? 'plain') !== 'S256')
      return redirectError(
        res,
        redirect_uri,
        'invalid_request',
    ...
    if (!validResource(resource))
      return redirectError(
        ...
        redirect_uri,
        'invalid_target',
        `resource must be ${resourceUrl(config)}`,
        state,
      );
    const scope = (q.scope ?? USER_SCOPES.join(' ')).split(/\s+/).filter(Boolean);
    if (scope.some((s) => !USER_SCOPES.includes(s)))
      return redirectError(
        res,
        redirect_uri,
        'invalid_scope',
        `Allowed scopes: ${USER_SCOPES.join(' ')}`,
        state,
      );
    ...
    const pending: PendingRequest = {
      ...
      redirect_uri,
      code_challenge,
      resource: resource ?? resourceUrl(config),
      scope,
      state,
    };
    ...
    await store.putAuth(
      ...
      now().getTime() + PENDING_TTL_MS,
    );
    res
      .set('cache-control', 'no-store')
      .type('html')
      .send(loginPage({ requestId, clientName, scopes: scope }));
  });
  ...
  router.post('/authorize', async (req, res) => {
    ...
    if (!raw)
      return res
        ...
        .send(
          messagePage(
            'Link expired',
            'This sign-in link has expired. Please start again from the Alexa app.',
          ),
        );
    ...
    if (householdName.length < 2 || passphrase.length < 6)
      return render('Please enter a household name and a passphrase of at least 6 characters.');

    const householdId = householdIdFromName(householdName);
    if (!householdId)
      return render('Please choose a household name with some letters or numbers in it.');
    let household = await store.getHousehold(householdId);
    if (household?.credential) {
      if (
        !safeEqual(hashPassphrase(passphrase, household.credential.salt), household.credential.hash)
      )
        return render('That passphrase does not match this household.');
    } else {
  ...
  router.post('/token', async (req, res) => {
    ...
    if (
      !creds ||
      creds.id !== config.oauthClientId ||
      !safeEqual(creds.secret, config.oauthClientSecret)
    ) {
    ...
    if (grantType === 'client_credentials') {
      ...
      if (requested.some((s) => s !== SCOPE_SERVICE))
        return oauthError(
          res,
          400,
          'invalid_scope',
          `client_credentials may only request ${SCOPE_SERVICE}`,
        );
      if (!validResource(body.resource))
        return oauthError(res, 400, 'invalid_target', `resource must be ${resourceUrl(config)}`);
      const access_token = await signAccessToken(
        keys,
        config,
        { sub: `client:${creds.id}`, client_id: creds.id, scope: SCOPE_SERVICE },
        SERVICE_TTL_S,
      );
      return res.json({
        access_token,
        token_type: 'Bearer',
        expires_in: SERVICE_TTL_S,
        scope: SCOPE_SERVICE,
      });
    }
    ...
    if (grantType === 'authorization_code') {
      ...
      if (resource && resource.replace(/\/+$/, '') !== grant.resource)
        return oauthError(
          res,
          400,
          'invalid_target',
      ...
      return res.json(
        await issueUserTokens(grant.household_id, creds.id, grant.scope, grant.resource),
      );
    }
    ...
    if (grantType === 'refresh_token') {
      ...
      if (requested.some((s) => !grant.scope.includes(s)))
        return oauthError(res, 400, 'invalid_scope', 'Cannot broaden scope on refresh');
      return res.json(
        await issueUserTokens(grant.household_id, creds.id, requested, grant.resource),
      );
    }
  ...
  async function issueUserTokens(
    ...
    const access_token = await signAccessToken(
      keys,
      config,
      { sub: householdId, client_id: clientId, scope: scope.join(' '), household_id: householdId },
      ACCESS_TTL_S,
    );
    const refresh_token = cryptoRandom(32);
    const grant: RefreshGrant = { client_id: clientId, household_id: householdId, scope, resource };
    await store.putAuth(
      `refresh:${sha256b64url(refresh_token)}`,
      JSON.stringify(grant),
      now().getTime() + REFRESH_TTL_MS,
    );
    return {
      access_token,
      token_type: 'Bearer',
