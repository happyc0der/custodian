...
export async function fetchJson<T>(url: string, opts: { fetch?: FetchLike; timeoutMs?: number; headers?: Record<string, string> } = {}): Promise<T> {
  ...
  try {
    const res = await f(url, {
      signal: ctrl.signal,
      headers: { accept: 'application/json', 'user-agent': 'custodian-recall-watch/0.1 (+https://github.com/happyc0der/custodian)', ...opts.headers },
    });
