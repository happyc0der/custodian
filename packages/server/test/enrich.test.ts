...
const fixture = <T>(name: string): T => JSON.parse(readFileSync(new URL(`./fixtures/${name}`, import.meta.url)).toString()) as T;
const t = '2026-09-11T12:00:00.000Z';
const item = (name: string, extra: Partial<Item> = {}): Item => ({ id: 'itm_1', household_id: 'dev', name, category: 'other', quantity: 1, aliases: [name], enrichment_status: 'pending', created_at: t, updated_at: t, ...extra });

function fakeClient(answers: Array<unknown>): ParseClient & { calls: Array<Record<string, unknown>> } {
...
describe('BedrockEnricher', () => {
  it('normalizes a spoken description into a product record via structured output', async () => {
    const client = fakeClient([{ brand: 'Graco', model: '4Ever DLX', category: 'car_seat', aliases: ['the car seat', 'Graco convertible seat'], canonical_name: 'Graco 4Ever DLX car seat' }]);
    const e = new BedrockEnricher({ bedrockRegion: 'us-west-2', bedrockModel: 'anthropic.claude-opus-5' }, client);
...
describe('Sweeper with an enricher', () => {
  const joolz = fixture<CpscRecall[]>('cpsc-car-seat.json').map(normalizeCpsc).find((r) => r.id === 'cpsc:26568')!;
  const sources: SweepSources = {
    cpsc: { async fetchSince() { return [joolz]; }, async searchProduct() { return [joolz]; } },
    nhtsaVehicle: { async fetchForItem() { return []; } },
    nhtsaChildSeat: { async fetchPage() { return { records: [], nextOffset: null, total: 0 }; } },
    openFda: { async fetchForItem() { return []; } },
  };
  ...
  it('applies normalization off the hot path and lets the model veto an ambiguous match', async () => {
    ...
    const client = fakeClient([
      { brand: 'Joolz', model: 'Aer2', category: 'stroller', aliases: ['the stroller'], canonical_name: 'Joolz Aer2 stroller' },
      { applies: 'no', confidence: 0.9, reason: 'Recall covers the car seat adapter accessory, not the stroller itself.' },
    ]);
    const enricher = new BedrockEnricher({ bedrockRegion: 'r', bedrockModel: 'm' }, client);
    const sweeper = new Sweeper({ store, sources, enricher, now: () => new Date(t), log: () => {} });
  ...
  it('enrichment failure is recorded and matching still runs deterministically', async () => {
    ...
    const failing: ParseClient = { messages: { parse: async () => { throw new Error('AccessDeniedException'); } } };
    const sweeper = new Sweeper({ store, sources, enricher: new BedrockEnricher({ bedrockRegion: 'r', bedrockModel: 'm' }, failing), now: () => new Date(t), log: () => {} });
