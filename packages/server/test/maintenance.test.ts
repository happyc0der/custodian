...
describe('maintenance defaults', () => {
  it('derives category rules from purchase/manufacture dates', () => {
    ...
    const smoke = defaultRulesFor(
      { ...base, category: 'smoke_detector', purchased_on: '2026-01-01' },
      '2026-09-11',
    );
    ...
    const seat = defaultRulesFor(
      { ...base, category: 'car_seat', manufactured_on: '2021-03-15' },
      '2026-09-11',
    );
    ...
    expect(
      defaultRulesFor(
        { ...base, category: 'appliance', purchased_on: '2026-09-01' },
        '2026-09-11',
      )[0],
    ).toMatchObject({ kind: 'warranty_ends', next_due: '2027-09-01' });
  });
});
...
describe('maintenance tools', () => {
  ...
  beforeAll(async () => {
    h = await startHarness({
      now,
      hooks: {
        onItemAdded: async (item) => void (await applyDefaultRules(h.deps.store, item, now())),
      },
    });
  });
  afterAll(() => h.close());
  ...
  it('adds default reminders when items are added', async () => {
    await callTool(h.client, 'add_item', {
      name: 'Kidde smoke detector',
      purchased_on: '2026-01-01',
    });
    await callTool(h.client, 'add_item', {
      name: 'Graco 4Ever car seat',
      manufactured_on: '2020-10-01',
    });
    await callTool(h.client, 'add_item', {
  ...
  it('log_maintenance rolls an interval reminder forward and clears one-shot ones', async () => {
    const r = await callTool<LogMaintenanceOutput>(h.client, 'log_maintenance', {
      item_name: 'brita filter',
    });
    expect(r.isError).toBe(false);
    expect(r.data.next_due).toBe('2027-03-12');
    expect(r.speech).toMatch(/again in about 6 months/);

    const ambiguous = await callTool<LogMaintenanceOutput>(h.client, 'log_maintenance', {
    ...
    const batteries = await callTool<LogMaintenanceOutput>(h.client, 'log_maintenance', {
      item_name: 'smoke detector',
      task: 'batteries',
      date: '2026-09-10',
    });
    ...
    const unit = await callTool<LogMaintenanceOutput>(h.client, 'log_maintenance', {
      item_name: 'smoke detector',
      task: 'replace_unit',
    });
    expect(unit.speech).toMatch(/one-time reminder/);
    expect((await h.deps.store.listRules('dev')).some((x) => x.kind === 'replace_unit')).toBe(
      false,
    );
  });

  it('set_maintenance_reminder creates custom recurring reminders', async () => {
    const r = await callTool<SetReminderOutput>(h.client, 'set_maintenance_reminder', {
      item_name: 'brita',
      label: 'Descale the pitcher',
      every_days: 90,
    });
    expect(r.data.rule).toMatchObject({
      kind: 'custom',
      next_due: '2026-12-10',
      days_until_due: 90,
    });
    expect(r.speech).toMatch(/in 90 days, and every 90 days after that/);
    const missing = await callTool<SetReminderOutput>(h.client, 'set_maintenance_reminder', {
      item_name: 'brita',
      label: 'Nothing',
    });
