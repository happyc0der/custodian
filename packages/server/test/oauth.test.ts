...
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- OAuth JSON bodies are asserted field by field below
...
describe('OAuth 2.1 authorization server (Alexa+ account linking shape)', () => {
  ...
  beforeAll(async () => {
    h = await startHarness({
      ...
      config: {
        ...
        oauthRedirectUris: [REDIRECT, REDIRECT_EU],
      },
    });
  ...
  async function pkceLogin(
    ...
    for (const [k, v] of Object.entries({
      ...
      response_type: 'code',
      scope: opts.scope ?? 'mcp:tools mcp:resources',
      state,
      code_challenge: challenge,
      code_challenge_method: 'S256',
      resource: opts.resource ?? mcpUrl,
    }))
    ...
    const login = await post(
      ...
      form({
        ...
        household: opts.household ?? 'The Rajput Home',
        passphrase: opts.passphrase ?? 'correct horse',
      }),
    );
    ...
    return {
      ...
      login,
      location,
      code: new URL(location || 'http://x/').searchParams.get('code'),
    };
  }

  it('serves RFC 8414 / RFC 9728 discovery documents in the shape Alexa+ checks at deploy time', async () => {
    const as = await fetch(`${h.baseUrl}/.well-known/oauth-authorization-server`).then(json);
    ...
    for (const p of [
      ...
      const prm = await fetch(`${h.baseUrl}${p}`).then(json);
      expect(prm).toMatchObject({
        resource: mcpUrl,
        authorization_servers: [h.baseUrl],
        bearer_methods_supported: ['header'],
      });
      expect(prm.scopes_supported).toEqual(
        expect.arrayContaining(['mcp:service', 'mcp:tools', 'mcp:resources']),
      );
    }
    const jwks = await fetch(`${h.baseUrl}/.well-known/jwks.json`).then(json);
  ...
  describe('tier 1: client_credentials', () => {
    it('issues a short-lived mcp:service token with no refresh token', async () => {
      ...
      const list = await h.rpc(
        { jsonrpc: '2.0', id: 1, method: 'tools/list', params: {} },
        { Authorization: `Bearer ${tok.access_token}` },
      );
      expect(list.status).toBe(200);
      const call = await h.rpc(
        {
          jsonrpc: '2.0',
          id: 2,
          method: 'tools/call',
    ...
    it('accepts client_secret_post and rejects wrong secrets or user scopes', async () => {
      const ok = await post(
        '/token',
        form({ grant_type: 'client_credentials', client_id: 'alexa', client_secret: secret }),
      );
      expect(ok.status).toBe(200);
      const bad = await post(
        '/token',
        form({ grant_type: 'client_credentials', client_id: 'alexa', client_secret: 'nope' }),
      );
      expect(bad.status).toBe(401);
      expect(await json(bad)).toMatchObject({ error: 'invalid_client' });
      const scope = await post(
        '/token',
        form({ grant_type: 'client_credentials', scope: 'mcp:tools' }),
        { authorization: basic },
      );
      expect(scope.status).toBe(400);
      expect(await json(scope)).toMatchObject({ error: 'invalid_scope' });
    });
  });
  ...
  describe('tier 2: authorization_code + PKCE', () => {
    ...
    it('rejects a wrong resource with invalid_target', async () => {
      ...
      expect(new URL(r.page.headers.get('location')!).searchParams.get('error')).toBe(
        'invalid_target',
      );
    });
    ...
    it('exchanges the code only with the right verifier, once, and binds the token to the household', async () => {
      ...
      expect(await json(wrongVerifier)).toMatchObject({ error: 'invalid_grant' });
      ...
      const add = await h.rpc(
        {
          jsonrpc: '2.0',
          id: 5,
          method: 'tools/call',
          params: { name: 'add_item', arguments: { name: 'Kidde smoke detector' } },
        },
        auth,
      );
      ...
      const tok2 = await json(refreshed);
      ...
      const broaden = await post(
        '/token',
        form({
          grant_type: 'refresh_token',
          refresh_token: tok2.refresh_token,
          scope: 'mcp:tools mcp:service',
        }),
        { authorization: basic },
      );
      expect(await json(broaden)).toMatchObject({ error: 'invalid_scope' });
    });

    it('the dev bearer is rejected when AUTH_MODE=oauth', async () => {
      const res = await h.rpc(
