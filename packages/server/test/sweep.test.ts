...
import type { AddItemOutput, BriefingOutput, CheckRecallsOutput, AcknowledgeRecallOutput, RecallDetailsOutput } from '@custodian/shared';
...
function fakeSources() {
  ...
  const sources: SweepSources = {
    cpsc: {
      async fetchSince() { calls.since++; return cpsc.map(normalizeCpsc); },
      async searchProduct(q: string) { calls.search++; return cpsc.filter((r) => r.Title.toLowerCase().includes(q.toLowerCase().split(' ')[0]!)).map(normalizeCpsc); },
    },
    nhtsaVehicle: { async fetchForItem(item) { calls.vehicle++; return item.vehicle?.model === 'Odyssey' ? odyssey.results.map(normalizeNhtsaVehicle) : []; } },
    nhtsaChildSeat: { async fetchPage(offset) { calls.seats++; return { records: normalizeChildSeatRecalls(seatPage.results), nextOffset: offset === 0 ? 100 : null, total: 200 }; } },
    openFda: { async fetchForItem() { calls.fda++; return []; } },
  };
...
describe('sweep + recall tools', () => {
  ...
  beforeAll(async () => {
    ...
    calls = fake.calls;
    h = await startHarness({ now, hooks: { onItemAdded: async (item) => void (await sweeper.sweepItem(item)) } });
  ...
  it('add_item answers immediately and the targeted sweep finds the recall afterwards', async () => {
    const added = await callTool<AddItemOutput>(h.client, 'add_item', { name: 'Joolz Aer2 car seat adapter' });
  ...
  it('check_recalls for one item scopes and disambiguates', async () => {
    const one = await callTool<CheckRecallsOutput>(h.client, 'check_recalls', { item_name: 'joolz adapter' });
    expect(one.data.scope).toBe('item');
    expect(one.data.matches).toHaveLength(1);
    const none = await callTool<CheckRecallsOutput>(h.client, 'check_recalls', { item_name: 'toaster' });
  ...
  it('vehicles get NHTSA campaigns via the targeted lookup', async () => {
    ...
    const r = await callTool<CheckRecallsOutput>(h.client, 'check_recalls', { item_name: 'Honda Odyssey' });
  ...
  it('get_recall_details speaks the remedy and contact; acknowledge closes the match', async () => {
    const details = await callTool<RecallDetailsOutput>(h.client, 'get_recall_details', { recall_id: 'cpsc:26568' });
    ...
    const ack = await callTool<AcknowledgeRecallOutput>(h.client, 'acknowledge_recall', { recall_id: 'cpsc:26568', action: 'remedy_requested' });
    expect(ack.data.match.status).toBe('remedy_requested');
    const after = await callTool<CheckRecallsOutput>(h.client, 'check_recalls', { item_name: 'joolz adapter' });
    expect(after.data.matches).toEqual([]);
    expect(after.speech).toMatch(/haven't found any recalls/);
    const withResolved = await callTool<CheckRecallsOutput>(h.client, 'check_recalls', { item_name: 'joolz adapter', include_resolved: true });
