...
describe('inventory tools', () => {
  ...
  it('lists inventory grouped for speech', async () => {
    ...
    expect(r.data.items.map((i) => i.category)).toEqual(
      expect.arrayContaining(['car_seat', 'stroller', 'vehicle']),
    );
    expect(r.speech).toMatch(/You have 4 items on file/);
    const filtered = await callTool<ListInventoryOutput>(h.client, 'list_inventory', {
      category: 'vehicle',
    });
    expect(filtered.data.items).toHaveLength(1);
    expect(filtered.data.items[0]!.vehicle).toEqual({
      make: 'Honda',
      model: 'Odyssey',
      year: 2019,
    });
  ...
  it('asks for disambiguation when a removal is ambiguous, and removes on an exact name', async () => {
    ...
    const adapterOnly = await callTool<RemoveItemOutput>(h.client, 'check_recalls', {
      item_name: 'car seat adapter',
    });
    expect(
      (adapterOnly.data as unknown as { scope: string; item?: { name: string } }).item?.name,
    ).toBe('Joolz Aer2 car seat adapter');
    const seats = await callTool<RemoveItemOutput>(h.client, 'check_recalls', {
      item_name: 'car seat',
    });
    ...
    const specific = await callTool<RemoveItemOutput>(h.client, 'check_recalls', {
      item_name: 'joolz adapter',
    });
    expect(specific.isError).toBe(false);
    expect((specific.data as unknown as { scope: string }).scope).toBe('item');

    const removed = await callTool<RemoveItemOutput>(h.client, 'remove_item', {
      name: 'Joolz Aer2 stroller',
    });
  ...
  it('answers every tool call well inside the Alexa+ latency budget', async () => {
    const res = await fetch(`${h.baseUrl}/metrics.json`).then(
      (r) => r.json() as Promise<{ tools: Record<string, { p95: number; count: number }> }>,
    );
