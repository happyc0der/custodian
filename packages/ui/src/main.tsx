...
import type { AcknowledgeRecallOutput, BriefingOutput, CheckRecallsOutput, ListInventoryOutput, LogMaintenanceOutput, MaintenanceView, RecallDetailsOutput, RecallMatchView, WhatsDueOutput } from '@custodian/shared';
import { app, applyHost, callTool, expand, openLink, tellModel, widthClass, type ToolResult } from './host.js';
...
function viewFor(toolName: string | undefined, result: ToolResult): View {
  if (result.isError) return { kind: 'error', text: result.content?.find((c) => c.type === 'text')?.text ?? 'Something went wrong.' };
  const d = (result.structuredContent ?? {}) as Record<string, unknown>;
  switch (toolName) {
    case 'check_recalls': return { kind: 'recalls', data: d as unknown as CheckRecallsOutput };
    case 'get_recall_details': return { kind: 'detail', data: d as unknown as RecallDetailsOutput };
    case 'list_inventory': return { kind: 'inventory', data: d as unknown as ListInventoryOutput };
    case 'whats_due': return { kind: 'maintenance', data: d as unknown as WhatsDueOutput };
    case 'household_briefing': return { kind: 'briefing', data: d as unknown as BriefingOutput };
  }
...
function Root() {
  ...
  const style = insets ? `--safe-top:${insets.top}px;--safe-right:${insets.right}px;--safe-bottom:${insets.bottom}px;--safe-left:${insets.left}px` : undefined;
  ...
  const patchMatch = (recallId: string, patch: Partial<RecallMatchView>) =>
    setView((v) => {
      if (v.kind === 'recalls') return { ...v, data: { ...v.data, matches: v.data.matches.map((m) => (m.recall.id === recallId ? { ...m, ...patch } : m)) } };
      if (v.kind === 'briefing') return { ...v, data: { ...v.data, new_recalls: v.data.new_recalls.map((m) => (m.recall.id === recallId ? { ...m, ...patch } : m)), open_recalls: Math.max(0, v.data.open_recalls - 1) } };
      if (v.kind === 'detail' && v.data.match?.recall.id === recallId) return { ...v, data: { ...v.data, match: { ...v.data.match, ...patch } } };
      return v;
    });

  const onAck = async (recallId: string, title: string, itemName: string, action: AckAction) => {
    const r = await callTool<AcknowledgeRecallOutput>('acknowledge_recall', { recall_id: recallId, action });
    if (r.isError) return notify(r.speech || 'Could not update that recall.');
    patchMatch(recallId, { status: action });
    tellModel(`On screen, the customer marked the recall "${title}" for their ${itemName} as ${action.replace('_', ' ')}.`);
    notify(r.speech);
  };
  ...
  const onDone = async (v: MaintenanceView) => {
    const r = await callTool<LogMaintenanceOutput>('log_maintenance', { item_name: v.item.name, task: v.kind });
  ...
  let body;
  switch (view.kind) {
    case 'loading': body = <div class="loading">Loading…</div>; break;
    case 'error': body = <div class="frame"><div class="empty"><div class="big" aria-hidden="true">😕</div><h2>{view.text}</h2></div></div>; break;
    case 'recalls': body = <RecallsView data={view.data} fullscreen={fullscreen} onExpand={expand} onDetails={onDetails} onAck={(m, a) => onAck(m.recall.id, m.recall.title, m.item.name, a)} />; break;
    case 'detail': body = <RecallDetailView data={view.data} onOpen={openLink} onBack={view.back ? () => setView(view.back!) : undefined} onAck={(id, a) => onAck(id, view.data.recall.title, view.data.match?.item.name ?? 'item', a)} />; break;
    case 'inventory': body = <InventoryView data={view.data} />; break;
    case 'maintenance': body = <MaintenanceView_ data={view.data} onDone={onDone} />; break;
    case 'briefing': body = <BriefingView data={view.data} fullscreen={fullscreen} onExpand={expand} onDetails={onDetails} onDone={onDone} onAck={(m, a) => onAck(m.recall.id, m.recall.title, m.item.name, a)} />; break;
  }
