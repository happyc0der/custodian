  ...
  return (
    <div class="row">
      ...
      <div class="main">
        ...
        <div class="meta">
          {v.item.name} · {formatDate(v.next_due)}
        </div>
      </div>
      {done ? (
...
export function MaintenanceView_({
  data,
  onDone,
}: {
  data: WhatsDueOutput;
    ...
    <div class="frame">
      ...
      {data.due.length === 0 && data.upcoming.length === 0 ? (
        <Empty
          icon="✅"
          title="All caught up"
          text={`Nothing due in the next ${data.horizon_days} days.`}
        />
      ) : (
        <>
          {data.due.length > 0 && (
            <Section title="Needs attention">
              <div class="list">
                {data.due.map((v) => (
                  <Row key={v.rule_id} v={v} onDone={onDone} />
                ))}
              </div>
            </Section>
          )}
