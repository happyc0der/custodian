  ...
  return (
    <article class="card" aria-label={m.recall.title}>
      {!compact && (
        <div class="media">
          {m.recall.image_url ? (
            <img src={m.recall.image_url} alt="" />
          ) : (
      ...
      <div class="body">
        ...
        {!compact && m.recall.remedy_options.length > 0 && (
          <div class="remedy">
            ...
            <span>{m.recall.remedy_options.join(' · ')}</span>
          </div>
        )}
      </div>
      <div class="actions">
        <Button primary onClick={() => onDetails(m)}>
          Details
        </Button>
        {!closed && !menu && (
          <Button ghost onClick={() => setMenu(true)}>
            I've handled it
          </Button>
        )}
        {!closed && menu && (
