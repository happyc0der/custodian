...
const LABEL: Record<string, string> = {
  ...
  tool: 'Tools',
  heater: 'Heaters',
  smoke_detector: 'Smoke & CO detectors',
  water_filter: 'Water filters',
  hvac_filter: 'Air filters',
  other: 'Other',
};
  ...
  return (
    <div class="frame">
      <Header
        ...
        sub={`${data.total} item${data.total === 1 ? '' : 's'}${data.open_recalls ? ` · ${data.open_recalls} open recall${data.open_recalls === 1 ? '' : 's'}` : ''}`}
      />
      {data.items.length === 0 ? (
        <Empty
          icon="📦"
          title="Nothing on file yet"
          text='Say "Alexa, I just bought a …" to start tracking.'
        />
      ) : (
            ...
            <div class="list">
              {items.map((i) => (
                <div class="row" key={i.id}>
                    ...
                    <div class="meta">
                      {[
                        ...
                        i.vehicle
                          ...
                          : undefined,
                        i.purchased_on ? `bought ${formatDate(i.purchased_on)}` : undefined,
                      ]
                        .filter(Boolean)
                        .join(' · ') || 'No details yet'}
                    </div>
                  ...
                  {recalls[i.id] ? (
                    <Badge kind="high">
                      {recalls[i.id]} recall{recalls[i.id] === 1 ? '' : 's'}
                    </Badge>
                  ) : (
