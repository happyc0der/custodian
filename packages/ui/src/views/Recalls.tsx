          ...
          {data.candidates.map((c) => (
            <div class="row" key={c.id}>
              ...
              <div class="main">
                ...
                <div class="meta">{[c.brand, c.model].filter(Boolean).join(' · ')}</div>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div class="frame">
      <Header
        ...
        sub={sub}
        right={
          !fullscreen && data.matches.length > 1 ? (
      ...
      {data.matches.length === 0 ? (
        <Empty
          ...
          text={
            data.last_sweep_at
              ? `Last checked ${new Date(data.last_sweep_at).toLocaleString()}`
              : 'Custodian keeps checking in the background.'
          }
        />
      ) : (
