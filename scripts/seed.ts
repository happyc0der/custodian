...
const ITEMS: Array<Record<string, unknown>> = [
  ...
  { name: '2019 Honda Odyssey', vehicle: { make: 'Honda', model: 'Odyssey', year: 2019 }, purchased_on: '2024-05-20' },
...
async function main() {
  ...
  const data = check.structuredContent as { matches: Array<{ item: { name: string }; recall: { id: string; title: string }; confidence: number }> };
  console.log(`\n${data.matches.length} recall match(es):`);
  for (const m of data.matches) console.log(`  ${m.confidence.toFixed(2)}  ${m.item.name}  ←  ${m.recall.id}  ${m.recall.title.slice(0, 70)}`);
  await client.close();
}
